> **Historical / superseded.** This is the original pre-implementation MVP design doc,
> written before most of the current schema and features existed. It describes a domain
> model (`Review`, `CustomList`, `CustomListItem`) that was never built in this shape, and
> package conventions from the earliest scaffold. It's kept for history only — see
> [`docs/architecture.md`](../architecture.md) for how the system actually works today.
